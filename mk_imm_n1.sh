...
INDEX_PATCH_HOME="${PWD}/immortal-files/immortal-index-patch"
...
# 20210330 add
DOCKER_PATCH="${PWD}/immortal-files/docker.patch"
...
# 20210704 add
SYSINFO_SCRIPT="${PWD}/immortal-files/30-sysinfo.sh"
...
# 20211019 add
FIRSTRUN_SCRIPT="${PWD}/immortal-files/first_run.sh"
...
echo -n "修改 openwrt-kernel 以配合自定义 banner ... "
...
echo "完成"
echo -n "固化 mac 地址 ... "
sed -i '/exit/d' ./etc/uci-defaults/99-default-settings
cat >> ./etc/uci-defaults/99-default-settings <<EOF
localMacaddr=\$(ifconfig | grep br-lan | awk '{print \$5}')
uci set network.lan.macaddr="\${localMacaddr}"
uci commit network

exit 0
EOF
echo "完成"
echo -n "调整 cpufreq 默认参数 ... "
[ -f ./etc/config/cpufreq ] && sed -i "/cpufreq/a\  option governor0 'schedutil'\n  option minfreq0 '100000'\n  option maxfreq0 '1512000'" ./etc/config/cpufreq
echo "完成"
echo -n "隐藏概览页 wifi ... "
sed -i '854 s/^/<!--/' ./usr/lib/lua/luci/view/admin_status/index.htm
sed -i '862 s/$/-->/' ./usr/lib/lua/luci/view/admin_status/index.htm
echo "完成"
echo "===================== 完成 ========================"
echo
